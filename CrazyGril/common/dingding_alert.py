...
def post_to_weixin(msg="default", Secret="", AppID=111111,
                   CropID=""):
    try:
        PartyID = 1
...
def judge(datas):
    ...
    print datas.split()[-2][-4:]
    print "judge ok"
    if datas[-1] == ";" or datas.split()[0] == "var" or datas.split()[1] or "now" or datas.split()[-2][
                                                                                     :4] == "Date" or datas.split()[
                                                                                                          -2][
                                                                                                      -4:] == "2018":
        return "1"
    else:
        ...
        try:
            ...
            dingding_alert(
                "Current time:\n{0};\nweb-api container has traceback:{1},\nplease attention!!\nAPI:{2}".format(
                    cur_time, json.dumps(traceback.format_exc()),
                    da))
