...
$('#ajax-content').on('click','button', function(e){
    ...
    $('#ajax-content').on('submit','.ajax-post', function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            error: function(xhr, test_status, error){
                $('#ajax-content').html(error);
            },
        });
    });
    
    $('#ajax-content').on('submit','.ajax-get', function(e){
        NProgress.start();
        e.preventDefault();
        var formdata = $(this).serialize();
        $.ajax({
...
$(document).ready(function () {
	...
	$('#ajax-content').on('click', 'a', function(e){
		if ($(this).hasClass('ajax-link')) {
			e.preventDefault();
			if ($(this).hasClass('add-full')) {
				$('#ajax-content').addClass('full-content');
			}
			else {
				$('#ajax-content').removeClass('full-content');
			}
			var url = $(this).attr('href');
			window.location.hash = url;
			LoadAjaxContent(url);
	...
	$('#ajax-body').on('click','button', function(e){
